...
const os = require('os');
const { dedupeKeepOrder } = require('../utils/tagUtils');
...
function discoverComfyInstallations() {
  ...
  // Secondary pass: targeted shallow scan for AppData Local roots.
...
function resolveComfyRoot(candidate) {
  ...
  // Some desktop installs keep nested ComfyUI directory.
...
function shouldDescend(name, depth) {
  ...
  // Depth 0-1: allow wider exploration in AppData roots, then narrow quickly.
  if (depth <= 1) {
    return true;
  }
  return lower.includes('comfy') || lower.includes('programs') || lower.includes('resources') || lower.includes('installs');
}
