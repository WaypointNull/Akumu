...
test('POST /api/regional/start validates input and starts a job', async (t) => {
  const server = await startServer(fakeDeps());
  t.after(() => server.close());

  const bad = await request(server, {
    method: 'POST',
    path: '/api/regional/start',
    headers: JSON_HEADERS,
    body: '{}'
  });
  assert.equal(bad.status, 400);
  assert.match(bad.json.error, /naturalLanguage is required/);
  ...
  assert.equal(ok.status, 200);
  assert.equal(ok.json.jobId, 'job_test');
});

test('GET /api/regional/status for an unknown job returns 404', async (t) => {
  const server = await startServer(fakeDeps());
  t.after(() => server.close());
